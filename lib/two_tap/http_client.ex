defmodule TwoTap.HTTPClient do
  ...
  @test_mode Application.get_env(:two_tap, :test_mode)
  ...
  def create_cart(products) do
    request(:post, "/cart", %{products: products, public_token: @public_token, test_mode: @test_mode}, [{"Content-Type", "application/json"}])
  end

  def get_cart_status(cart_id) do
    request(:get, "/cart/status", [], [], %{params: %{public_token: @public_token, cart_id: cart_id, test_mode: @test_mode}})
  end

  def start_purchase(cart_id, purchase_data) do
    request(:post, "/purchase", %{cart_id: cart_id, fields_input: purchase_data, confirm: purchase_callbacks, test_mode: @test_mode, public_token: @public_token}, [{"Content-Type", "application/json"}])
  end
